...
- Suffixes for clarity (optional):
  - U or u → unsigned
  - LL, ULL → long long or unsigned long long (used rarely in this course)
...
### Boolean Type
- C++ also includes the bool type (true / false), discussed in a later video.
...
- C++ provides **template functions** (covered later) to avoid duplicating logic for different types.
